...
export const useProducts = () => {
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: () => {
      sileo.success({ title: "Producto creado", description: "El producto ha sido agregado." });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      sileo.success({ title: "Producto eliminado", description: "El producto ha sido borrado." });
      queryClient.invalidateQueries({ queryKey: ['products'] });
    },
    onError: (error: Error) => {
